...
class MySwinFormerBlocks(nn.Module):
    def __init__(self,
        ...
        # stochastic depth
        dpr = [x.item() for x in
               torch.linspace(0, self.drop_out_rate, sum(self.blocks_depths))]  # stochastic depth decay rule
...
if __name__ == "__main__":
    input_feature = torch.randn(2, 15, 256, 256).cuda()

    swinFormerBlocks = MySwinFormerBlocks(input_feature_size=(256, 256),
                                          input_feature_channels=15,
                                          skiped_patch_embed=False,
    ...
    output_feature = swinFormerBlocks(input_feature)[3]
