...
def build_scheduler(config, optimizer):
    ...
    """
    # scheduler = {'interval': configs.TRAINER.SCHEDULER_INTERVAL}
    ...
    #     scheduler.update(
    #         {'scheduler': MultiStepLR(optimizer, configs.TRAINER.MSLR_MILESTONES, gamma=configs.TRAINER.MSLR_GAMMA)})
    #elif name == 'CosineAnnealing':
    #    scheduler = CosineAnnealingLR(optimizer, configs.num_steps+100)
    #     scheduler.update(
    #         {'scheduler': CosineAnnealingLR(optimizer, configs.TRAINER.COSA_TMAX)})
    # elif name == 'ExponentialLR':
    #     scheduler.update(
    #         {'scheduler': ExponentialLR(optimizer, configs.TRAINER.ELR_GAMMA)})
    else:
