...
class Mydataset(FlowDataset):
    def __init__(self, aug_params=None, split='train', root=r'F:\flow_dataset\flow_tutor_datasets'):
        super(Mydataset, self).__init__(aug_params)
        image_root = osp.join(root, split)

        for setup in os.listdir(image_root):
            image_list = sorted(glob(osp.join(image_root, setup, '*.png')))
            for i in range(len(image_list)-1):
                self.image_list += [ [image_list[i], image_list[i+1]] ]
                self.extra_info += [ (setup, i) ] # scene and frame_id
