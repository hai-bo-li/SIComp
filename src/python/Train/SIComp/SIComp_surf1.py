...
def train(cfg):
    ...
    scheduler_SCNet_surf1 = get_scheduler(SCNet_surf1_Opt, cfg.trainer.comp_scheduler)
    ...
    for epoch in tqdm(range(cfg.trainer.num_epochs), desc='train_epoch'):
        for i_batch, data_blob in enumerate(train_loader):
            ...
            SCNet_surf1_Opt.zero_grad()
            cam_crop, prj_GT, surfs_crop = [x.to(device) for x in data_blob]
            warped_predict = OmniCompNet_surf1(prj_GT, cam_crop, surfs_crop)
            ...
            if isinstance(scheduler_flowformer, LambdaLR):
                ...
                scheduler_flowformer.step()
            if isinstance(scheduler_SCNet_surf1, LambdaLR):
                scheduler_SCNet_surf1.step()
            else:
                scheduler_SCNet_surf1.step()
            ...
            if total_steps % cfg.test.test_vis_num == 0:
                ...
                save_config_and_model_info(cfg, SCNet_surf1, save_metrics_folder)

                vis.line(
                    X=np.array([[total_steps] * len(metrics)]),
                    Y=np.array([[test_mae, test_rmse, train_loss.item(), test_ssim, 0.005 * test_diff_map, test_lpips]]),
                    win=win_metrics,
                    update='append'
                )
            ...
            total_steps += 1
            save_one_model_checkpoint(OmniCompNet_surf1, total_steps, cfg.trainer.save_num, save_checkpoint_folder, current_time, is_final=False)
        ...
        if not isinstance(scheduler_flowformer, LambdaLR):
            scheduler_flowformer.step()
        if not isinstance(scheduler_SCNet_surf1, LambdaLR):
            scheduler_SCNet_surf1.step()
...
if __name__ == '__main__':
    ...
    SCNet_surf1 = SCNet_surf1().to(device)
    model_name = SCNet_surf1.__class__.__name__
    SCNet_surf1.load_state_dict(torch.load(cfg.SCNet))
    OmniCompNet_surf1 = Connection(FlowFormer, SCNet_surf1, warp_func=warp_images).to(device)
