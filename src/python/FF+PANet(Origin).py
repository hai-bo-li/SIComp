...
from configs.local.FF_PANet_origin_Compensation_Image_config import get_cfg
...
if __name__ == '__main__':
    ...
    if args.mode == "actual":
        FF_PANet = FlowCompensationModel(FlowFormer, PANet, warp_func=warp_images, actual_compensation=True).to(device)
        FF_PANet.load_state_dict(torch.load(cfg.FF_PANet_origin_pretrain_path))
        FF_PANet.eval()
        ...
        for data_name in datasets_module.valid_data_lists:
            ...
            if device.type == 'cuda':
                torch.cuda.reset_peak_memory_stats(device=device)  # Reset CUDA memory statistics
            with torch.no_grad():
                # Iterate over all datasets simultaneously
                for data_blob, desire_test in tqdm(zip(test_loader, desire_test_loader),
                                                   total=np.ceil(200 / cfg.batch_size)):
                    # Normalize the remaining images
                    ...
                    prj_cmp_test = FF_PANet(prj_GT, cam_crop, surfs_crop, desire_test)
                    # Save images and update the counter
        ...
        FF_PANet = FlowCompensationModel(FlowFormer, PANet, warp_func=warp_images).to(device)
        FF_PANet = FF_PANet.to(device)
        ...
        for data_name in datasets_module.valid_data_lists:
            ...
            with torch.no_grad():
                ...
                if (h_t, w_t) != FF_PANet.train_size:
                    f_cached = predict_flow_highres(
                        FF_PANet.flow_model, gt_train, cam_train,
                        target_size=(h_t, w_t), train_size=FF_PANet.train_size
                    )
                else:
                    f_cached = FF_PANet.flow_model(gt_train, cam_train)

                # Store the dataset-specific optical flow in fixed_flow
                FF_PANet.fixed_flow = f_cached.detach()
            ...
            with torch.no_grad():
                ...
                for i_batch, data_blob in tqdm(enumerate(test_loader), total=len(test_loader), desc=f"Testing {data_name}:"):
                    ...
                    predicted = FF_PANet(GT=prj_GT, x=cam_crop, s=surfs_crop)
